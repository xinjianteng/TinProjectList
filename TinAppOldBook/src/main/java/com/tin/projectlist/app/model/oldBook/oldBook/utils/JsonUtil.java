package com.tin.projectlist.app.model.oldBook.oldBook.utils;
...
public class JsonUtil {
	private static final String LOG_JSON_ERROR = "JsonError";
	...
	/**
	 * 根据给定包含数组对象的json，将其解析成相应的JSONArray
	 * @param json
	 * 				给定的字符串
	 * @param key
	 * 				要解析出的对象关键字
	...
	public static JSONArray getJsonArray(String json,String key){
		...
		try {
			...
			object=result.getJSONArray(key);
	 ...
	 * @return 返回一个字符串，表示根据指定的key所得到的值，获取失败或发生JSON解析错误则返回空字符串
	 */
	public static Boolean getJsonBooleanByKey(String json, String key) {
		boolean value = false;
		try {
			JSONObject jo = new JSONObject(json);
	 ...
	 * @return 返回一个字符串，表示根据指定的key所得到的值，获取失败或发生JSON解析错误则返回空字符串
	 */
	public static Integer getJsonIntByKey(String json, String key) {
		Integer value = 0;
		try {
			JSONObject jo = new JSONObject(json);
			value = jo.getInt(key);
	...
	public static boolean isJsonNull(String json) {
		return json == null || json.equals("") || json.equals("null")
