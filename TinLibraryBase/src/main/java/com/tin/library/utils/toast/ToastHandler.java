package com.tin.library.utils.toast;
...
final class ToastHandler extends Handler {
        ...
        switch (msg.what) {
            case TYPE_SHOW:
                ...
                if (text != null) {
                    ...
                    mToast.show();
                    // 等这个 Toast 显示完后再继续显示
                    sendEmptyMessageDelayed(TYPE_CONTINUE, getToastDuration(mToast, text) + 1000);
                } else {
                    isShow = false;
                }
                break;
     ...
     * 根据文本来获取吐司的显示时间
     */
    private static int getToastDuration(Toast toast, CharSequence text) {
        // 如果显示的文字超过了10个就显示长吐司，否则显示短吐司
        return text.length() > 20 ? ToastHelper.LONG_DURATION_DELAY : ToastHelper.SHORT_DURATION_DELAY;
    }
}