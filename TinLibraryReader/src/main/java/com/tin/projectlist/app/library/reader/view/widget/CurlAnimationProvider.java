...
/**
 * 类名： CurlAnimationProvider.java<br>
 * 描述： 翻页动画<br>
 * 创建者： jack<br>
 * 创建日期：2013-4-26<br>
 * 版本：  <br>
...
class CurlAnimationProvider extends AnimationProvider {
	...
	protected void drawInternal(Canvas canvas) {
		if (myUseCanvasHack) {
			...
			if (myBuffer == null ||
				...
				myBuffer = Bitmap.createBitmap(myWidth, myHeight, getBitmapTo()[0].getConfig());
			}
	...
	private void drawInternalNoHack(Canvas canvas) {
		...
		final Bitmap fgBitmap = getBitmapFrom()[0];
		...
		myEdgePaint.setColor(GBAndroidColorUtil.rgb(GBAndroidColorUtil.getAverageColor(fgBitmap)));
//		myEdgePaint.setColor(Color.BLUE);
...
//		canvas.drawCircle(x, y, 5, myPaint);
		...
		canvas.drawCircle(cornerX, cornerY, 5, myPaint);
		myPaint.setColor(Color.RED);
		canvas.drawCircle(myStartX, myStartY, 5, myPaint);
		myPaint.setColor(Color.BLACK);
		canvas.drawCircle(myEndX, myEndY, 5, myPaint);
