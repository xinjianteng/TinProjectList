...
//	}
	static DeflatingDecompressorInterface  mInterface;
	public static void setmInterface(DeflatingDecompressorInterface mInterface) {
		DeflatingDecompressor.mInterface = mInterface;
	}
	// common variables
	...
	void reset(MyBufferedInputStream inputStream, LocalFileHeader header) throws IOException {
		if (myInflatorId != -1) {
			mInterface.endInflating(myInflatorId);
			myInflatorId = -1;
		}
		...
		myInflatorId = mInterface.startInflating();
		if (myInflatorId == -1) {
	...
	private void fillOutBuffer() throws IOException {
		...
		while (myOutBufferLength == 0) {
			...
			final long result =mInterface.inflate(myInflatorId, myInBuffer, myInBufferOffset, myInBufferLength, myOutBuffer);
			if (result <= 0) {
