...
public class SharePreferenceUtil {
    ...
    private static Map<String, SharePreferenceUtil> sPreferences = new HashMap<String, SharePreferenceUtil>();
    // Migrate Map
    ...
    private SharePreferenceUtil(Context context, String prefName) {
    ...
    public static SharePreferenceUtil getInstance(Context context, String prefName) {
        synchronized (sInitLock) {
            if (sPreferences.get(prefName) == null) {
                try {
                    sPreferences.put(prefName, new SharePreferenceUtil(context, prefName));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    private String doMigrate(String key) {
        ...
        if (isExistSharedPreferenceFile(mAppContext, key, mAppDataDir)) {
            Map<String, ?> map = mAppContext.getSharedPreferences(key, 0)
                    .getAll();
            Object temp = null;
            for (Map.Entry<String, ?> entry : map.entrySet()) {
    ...
    public static void save(boolean async) {
        SharePreferenceUtil temp;
        synchronized (sInitLock) {
            for (Map.Entry<String, SharePreferenceUtil> entry : sPreferences.entrySet()) {
                temp = entry.getValue();
