package com.example.nettytest.meibeike.communication.entity;


import com.example.nettytest.meibeike.communication.util.ByteUtil;
...
public class PackageHeader {
  ...
  /**
   * 包头构造函数
   * @param inclusionLenth 包体长度
   * @param encryption 加密类型 0 为不加密
   * @param serialNumber 流水号
   * @param fileType 包类型
   * @param mainFunction 主功能号，即模块功能号
   * @param headerFunction 
   * @param function  业务功能号
   * @param subversion 业务功能版本号
   * @param retain 保留字
   */
  public PackageHeader(long inclusionLenth, int encryption, int serialNumber, int fileType, int mainFunction,
      int headerFunction, int function, int subversion, int retain) {
...
//    this.retain = getRetain(readBuffer,pos);
  ...
  public String toString() {
    return "inclusionLenth = " + inclusionLenth + ";  encryption = " + encryption + ";  serialNumber = " + serialNumber
        + ";  fileType = " + fileType + ";  function = " + function
