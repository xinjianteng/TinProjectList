...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private boolean mDividerIndicate = true;
    private int mUnderlineWidth = 0;
    private boolean mUnderlineCircle;
    private int mDividerWidth = 0;
    private int mDividerPadding = 0;
    private int mDividerColor;

    private int mTabPadding = 12;
    private int mTabTextSize = 48;
    private int mTabTextSizeUnSelected = mTabTextSize;
    ...
    private int mTabBackgroundResId = R.drawable.selector_bg_psts_tab;

    private IPreClick mOnPreClickListener;

    public interface IPreClick {

        boolean ableChangPage(int position);
    }

    private IScrolling mOnScrollListener;

    public interface IScrolling {

        void onScrolling(int x, int y, int oldX, int oldY);
    }
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTabTextSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, mTabTextSize, dm);
        mTabTextSizeUnSelected = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, mTabTextSize, dm);
        ...
        int textPrimaryColor = a.getColor(TEXT_COLOR_PRIMARY, getResources().getColor(android.R.color.transparent));
        ...
        mUnderlineHeight = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsUnderlineHeight,
                mUnderlineHeight);
        mUnderlineWidth = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsUnderlineWidth,
                mUnderlineWidth);
        mDividerIndicate = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsDividerIndicate, mDividerIndicate);
        ...
        mUnderlineCircle = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsUnderlineCircle, false);
        ...
        mTabTextSizeUnSelected = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsunTabTextSize, mTabTextSize);
        ...
        if (mTabTextColor == null) {
            mTabTextColor = createColorStateList(textPrimaryColor, textPrimaryColor,
                    Color.argb(tabTextAlpha, Color.red(textPrimaryColor),
    ...
    public void setOnScrollListener(IScrolling onScrollListener) {
        this.mOnScrollListener = onScrollListener;
    }

    public void setOnPreClickListener(IPreClick onPreClickListener) {
    ...
    public void setViewPager(ViewPager pager) {
        ...
        if (!mAdapterObserver.isAttached()) {
            pager.getAdapter()
                    .registerDataSetObserver(mAdapterObserver);
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (isCustomTabs) {
                ...
                tabView = LayoutInflater.from(getContext())
                        .inflate(R.layout.view_psts_tab, this, false);
            }
            CharSequence title = mPager.getAdapter()
                    .getPageTitle(i);
    ...
    private void addTab(final int position, CharSequence title, View tabView) {
        ...
        if (textView != null) {
            if (title != null) {
                ...
                textView.setTypeface(mTabTextTypeface);
            }
        }
        ...
        tabView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                boolean ableChangePage = true;
                if (mOnPreClickListener != null) {
                    ableChangePage = mOnPreClickListener.ableChangPage(position);
                }

                if (ableChangePage && mPager.getCurrentItem() != position) {
    ...
    private void scrollToChild(int position, int offset) {
        ...
        int newScrollX = mTabsContainer.getChildAt(position)
                .getLeft() + offset;
    ...
    protected void onScrollChanged(int x, int y, int oldx, int oldy) {
        ...
        if (mOnScrollListener != null) {
            mOnScrollListener.onScrolling(x, y, oldx, oldy);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIndicatorHeight > 0) {
            ...
            if (mUnderlineWidth > 0) {
                float width = lines.first + mPaddingLeft + lines.second + mPaddingLeft;
                float startLeft = width / 2 - mUnderlineWidth / 2;
                float endRight = width / 2 + mUnderlineWidth / 2;
                if (mUnderlineCircle) {
                    canvas.drawRoundRect(new RectF(startLeft, height - mIndicatorHeight, endRight, height),
                            mIndicatorHeight / 2, mIndicatorHeight / 2, mRectPaint);
                } else {
                    canvas.drawRect(startLeft, height - mIndicatorHeight, endRight, height,
                            mRectPaint);
                }
            } else {
                if (mUnderlineCircle) {
                    canvas.drawRoundRect(new RectF(lines.first + mPaddingLeft, height - mIndicatorHeight, lines.second + mPaddingLeft, height),
                            mIndicatorHeight / 2, mIndicatorHeight / 2, mRectPaint);
                } else {
                    canvas.drawRect(lines.first + mPaddingLeft, height - mIndicatorHeight, lines.second + mPaddingLeft, height,
                            mRectPaint);
                }
            }
        }
    ...
    private class PageListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            int offset = mTabCount > 0 ?
                    (int) (positionOffset * mTabsContainer.getChildAt(position)
                            .getWidth()) :
                    0;
        ...
        public void onPageScrollStateChanged(int state) {
            ...
            if (mPager.getCurrentItem() + 1 <= mPager.getAdapter()
                    .getCount() - 1) {
    ...
    private void unSelect(View tab) {
        if (tab != null) {
            ...
            if (tab_title != null) {
                ...
                tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTabTextSizeUnSelected);
            }
            if (isCustomTabs) {
    ...
    protected void onDetachedFromWindow() {
        ...
        try {
            if (mPager != null) {
                if (mAdapterObserver.isAttached()) {
                    mPager.getAdapter()
                            .unregisterDataSetObserver(mAdapterObserver);
